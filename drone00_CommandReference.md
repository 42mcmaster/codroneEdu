...
Every program you write this unit has the same four bookends.
...
**In Python for Robolink you do not type these.** Every new file comes with them already written. You add your code to the middle.

`drone = Drone()` makes an **object**. Every command after that is called on it, which is why they all start with `drone.` — `drone.takeoff()`, `drone.land()`, and so on. The browser editor requires the object to be named `drone` and will not accept another name.

If you delete one of the starter lines by accident, opening a new tab brings it back.
...
Before `pair()` can work, the editor itself has to be connected: click **Connect** in the connection window in the bottom left corner, pick your drone in the popup, and wait for the window to turn green. If it is not green, no program will run.
...
**You have three ways to stop the drone, and `emergency_stop()` is the weakest of them.** A command in your program only fires when the program reaches that line.

| Where | How | What happens |
|---|---|---|
| Controller | Hold `L1`, pull the left joystick down | Motors cut. Works no matter what the program is doing. |
...
| Run does nothing at all | The connection window in the bottom left is not green. Click Connect. |
